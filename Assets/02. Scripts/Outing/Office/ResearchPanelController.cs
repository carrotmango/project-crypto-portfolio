...
public class ResearchPanelController : MonoBehaviour {
    ...
    private string FormatMarketCap(double amount, bool isUsd) {
        ...
        if (isUsd) {
            ...
            // [한글 + 원화 모드] 한국인들에게 익숙한 조, 억, 만
            if (displayAmount >= 1_0000_0000_0000) return $"{(displayAmount / 1_0000_0000_0000):F2}조";
            else if (displayAmount >= 1_0000_0000) return $"{(displayAmount / 1_0000_0000):F0}억";
            else if (displayAmount >= 1_0000) return $"{(displayAmount / 1_0000):F0}만";
            else return $"{displayAmount:N0}";
        }
    }
